 ...
 */
ATest.prototype.getStatus = function () {
	return this._status;
};
/**
	...
	try {
		...
		if (!waitForThis) {
			if (this._status === 'waiting') {
				waitForThis = function waitForThis() {
					return atest._status === 'pass' || atest._status === 'fail';
				};
			} else {
		...
		// waitFor?
		var testDoneFn = function testDoneFn(yes) {
		...
		SJTest.waitFor(waitForThis, testDoneFn, timeout || 10000, timeoutFn);
	} catch (error) {
		this.error = error;
...
SJTest.run = function (testSet) {
	...
	assert((typeof testSet === 'undefined' ? 'undefined' : _typeof(testSet)) === 'object', testSet);
...
SJTest.runTest = function (testName, testFn, waitForThis, timeout) {
	...
	if (testFn) {
		...
		if (!dtest) {
			// Fail -- bad name
			dtest = new ATest(testName, function () {
				throw "Could not find test: " + testName;
			});
		}
	}
	...
	var skip = false;
	if (testFn) {
		// don't skip "manual" run-by-name calls
		// Skip if name begins //
...
SJTest.display = function () {
	...
	if (!SJTest._displayPanel || !SJTest._displayPanel.length) {
		...
		SJTest._displayPanel = SJTestUtils.$create("<div id='SJTestDisplay' " + (SJTest.styling ? "style='z-index:100000;background:white;border:2px solid black;position:fixed;top:0px;right:0px;width:70%;overflow:auto;max-height:100%;'" : '') + " class='SJTest panel panel-default'></div>");
...
SJTest.isa = function (obj, klass) {
	...
	for (var i = 0; i < 10; i++) {
		// limit the recursion 10-deep for safety
...
SJTest.match = function (value, matcher) {
	...
	if (typeof matcher === 'string') {
		...
		for (var mi = 0; mi < ms.length; mi++) {
			...
			if (m.substr(m.length - 2, m.length) === '[]') {
				...
				var arrayType = m.substr(0, m.length - 2);
	...
	if ((typeof matcher === 'undefined' ? 'undefined' : _typeof(matcher)) === 'object' && (typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object') {
...
SJTest.expectTests = function (n, timeout) {
	...
	SJTest.runTest("expectTests_" + n, function () {}, function () {
		return SJTest.tests.length > n;
	}, timeout);
};
  ...
  */
	SJTestUtils.str = function (obj) {
		...
		try {
			...
			for (var p in obj) {
				var v = obj[p];
				if (typeof v == 'function') continue;else safe[p] = "" + v;
			}
			return JSON.stringify(safe);
		}
	}; // str()
